...
$.ajax({
    ...
    success: function(data) {
      ...
      let labels2=[]
for (var i = 0;i<data.length;i++){
        labels2.push(data[i].bestSellingCustomer)
        labels2.push(data[i].bestSellingStore)
        labels2.push(data[i].bestSellingCategory)
        labels2.push(data[i].bestSellingProduct)
        labels1.push(data[i].bestSellingCustomerX)
        labels1.push(data[i].bestSellingCategoryX)
        labels1.push(data[i].bestSellingProductX)
        labels1.push(data[i].bestSellingStoreX)
     }
...
$.ajax({
    ...
    dataType: 'json',
    type: 'POST',
    url: "/dashChartStoreG4",
    success: function(data) {
      console.log(data,'data')
      let labels1=[]
      let labels2=[]
for (var i = 0;i<data.length;i++){
        labels2.push(data[i].qty)
 
        labels1.push(data[i].customer)
...
/*
var button = document.getElementById('myChart5tab').addEventListener('click', function Click(name){
 

    
      
     // let customer = 'Pick n Pay'
      let customer = document.getElementById('cust').textContent
